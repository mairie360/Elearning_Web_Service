...
// ---------------------------------------------------------------------------
// Test de charge k6 pour le front Elearning (Elearning_Web_Service).
...
// /health, puis les lectures /elearning/catalog et /elearning/profile relayées par le proxy same-origin vers BFF Elearning.
...
export const options = {
  ...
  thresholds: {
    ...
    'http_req_duration{endpoint:elearning}': ['p(95)<500'], // proxy + agrégation BFF + upstream
  },
};
...
export default function (data) {
  ...
  const dataParams = { headers: { ...cookie, Accept: 'application/json' }, redirects: 0, tags: { endpoint: 'elearning' } };
  ...
  group('elearning reads', () => {
    const elearningCatalog = http.get(`${BASE_URL}/elearning/catalog`, dataParams);
    check(elearningCatalog, { '/elearning/catalog 200': (r) => r.status === 200 });

    const elearningProfile = http.get(`${BASE_URL}/elearning/profile`, dataParams);
    check(elearningProfile, { '/elearning/profile 200': (r) => r.status === 200 });
  });
