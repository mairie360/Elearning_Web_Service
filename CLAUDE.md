...
`contracts/openapi.json` is a committed copy of BFF_Elearning's contract and `src/contracts/bff.d.ts` is generated from it (`openapi-typescript@7.10.1`, pinned in `scripts/contracts.mjs`). Never hand-edit either file.
...
- **Client calls** — pages call same-origin paths (e.g. `/elearning/*`) through clients that parse `{ error: { message } }` / `{ message }` bodies into typed errors and, when no Authorization header is set, add a Bearer token stored in `localStorage` (`mairie360.auth.jwt`, see `src/lib/auth-token.ts`); in normal use the proxy relies on the cookie.
- `src/app/page.tsx` mounts `ElearningModule`, `src/app/profile/page.tsx` mounts `ProfileModule`; loading, actions and navigation live in `src/features/elearning/` (`appData.ts` holds the data mapping).
